use crate::graphics::shape::{DrawableShape, ShapeBatch};
...
impl<'a> Canvas<'a> {
    ...
    pub fn draw_shape(&mut self, shape: DrawableShape) {
        match self.commands.last_mut() {
            Some(CanvasCommand::DrawShapes(batch)) if &batch.shape == shape.shape => {
        ...
        self.graphics.shape_renderer.add(shape.as_shape_instance());
    }
                ...
                match command {
                    CanvasCommand::UpdateCamera => {
                        let projection_bind_group =
                            &self.graphics.camera_manager[projection_bind_group_index];
